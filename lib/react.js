...
export async function perform(channel, user, cmd, args, client) {
    ...
    if (!automod(user, cmd, args, channel, client, permitted, perm_time)) {
        ...
        if (cmd === 'crazy' || args.includes('crazy') && !user.username.includes('interyon') && Date.now() < crazy_cooldown || crazy_cooldown == 0) {
            misc.crazy(channel, client);
            crazy_cooldown = Date.now() + 600000;
        }
        ...
        if (cmd === ':3') {
            client.say(channel, `:333 ${user.username}`);
        }

        if (cmd === '!about')
        {
            client.say(channel,
                `/me The official 23 Crowns Twitch Bot. Developed by interyon. interyPOP ${version}`);
        }
        ...
        //23Crowns Commands
        if (CROWNS.includes(channel.substring(1))){
            if (cmd === '!pride'){
                client.say(
                    channel,
                    `/me Parallel Pride. The sussiest of the Crowns`);
            }
            if (cmd === '!stract'){
                client.say(channel,
                    `/me The BEST 16 year old "HACKER"`);
            }
            if (cmd === '!deano' || cmd === '!deanobeano'){
                client.say(channel,
                    `/me Clipped Twyman on drums :clown:`);
            }
            if (cmd === '!swpr'){
                client.say(channel,
                    `/me Swpr no Swpng`);
            ...
            if (cmd === '!alastor'){
                client.say(channel,
                    `/me The man that does it all`);
            }
            if (cmd === '!omni' || cmd === '!omnibal'){
                client.say(channel,
                    `/me balls.`);
            }
            if (cmd === '!roy'){
                client.say(channel,
                    `/me Ocean man, take me by the hand. Lead me to the land that you understand`);
            }
            if (cmd === '!chai'){
                client.say(channel,
                    `/me brett`);
            ...
            if (cmd === '!bams'){
                client.say(channel,
                    `/me stop spinning dude you're not a trickshotter`);
            }
            if (cmd === '!twyman'){
                client.say(channel,
                    `/me Got clipped by Deano on drums`);
            }
            if (cmd === '!rymms'){
                client.say(channel,
                    `/me is a frog`);
            ...
            if (cmd === '!chiefz'){
                client.say(channel,
                    `/me got cummed on by a fish`);
            }
            if (cmd === '!zephy'){
                client.say(channel,
                    `/me hot spicy pepper daddy`);
            }
            if (cmd === '!razz'){
                client.say(channel,
                    `/me KingGreen97`);
            ...
            if (cmd === '!fhai'){
                client.say(channel,
                    `/me The king of Cold War :crown:`);
            }
            if (cmd === '!zelta'){
                client.say(channel,
                    `/me Biggest nerd to never touch grass`);
            }
            if (cmd === "!interyon") {
