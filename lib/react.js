...
export function perform(channel, user, cmd, args, client){
  ...
  automod(user, args, channel, client);
}

function automod(user, args, channel, client){
  ...
  if (shouldSendMessage){
    ...
    .then((data) => {
      ...
      db.run("UPDATE users SET Warnings = Warnings + 1 WHERE UserID = ?", user['user-id'], err => {
        if (err){
          console.log(`Could not add warning`.red);
        }else{
          console.log(`${user.username} has been warned`);
        }
      });
      //catch error deleting and print reason to chat for broadcaster
    }).catch((err) => {
...
function warn(channel, user, cmd, args, client){
  ...
  if (`#${user.username}` === channel || user.mod === true){
    if (!args[0]){
      ...
      new Promise(resolve => {
        ...
        if (warnings === 0){
          client.say(channel,
            `${target} has not received any warnings.`
          );
        }else{
          db.run("UPDATE users SET Warnings = Warnings - 1 WHERE UserName = ?", target);
          client.say(channel,
