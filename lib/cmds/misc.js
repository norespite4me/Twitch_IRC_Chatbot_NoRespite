...
export function myd(channel, user, cmd, args, client){
  ...
  if (!args[0]){
    if(user.username == 'interyon'){
      client.say(channel, `/me ${user.username}'s D is approximately 50 cm long! Squid1 Squid2 Squid3 Squid2 Squid4`);
    }else{
    ...
    if(args[0] == 'interyon' || args[0] == '@interyon'){
      client.say(channel, `/me ${args[0]}'s D is approximately 50 cm long! Squid1 Squid2 Squid3 Squid2 Squid4`);
    }else{
...
export function interyon(channel, cmd, client){
  client.say(channel, "/me PogChamp BEST BOT PogChamp BEST BOT PogChamp BEST BOT PogChamp BEST BOT PogChamp BEST BOT PogChamp BEST BOT PogChamp");
  console.log(`* Executed ${cmd} command`.green);
}
