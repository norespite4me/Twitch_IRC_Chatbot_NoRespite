...
export function interyon(channel, cmd, client){
  client.say(channel, "/me PogChamp BEST BOT PogChamp BEST MOD PogChamp BEST BOT PogChamp BEST MOD PogChamp BEST BOT PogChamp BEST MOD PogChamp");
  console.log(`* Executed ${cmd} command`.green);
}
...
export async function hit(channel, user, cmd, args, client){
  ...
  client.say(channel,
    `/me ${user.username} has hit ${target} with a ${randitem()}`);
  console.log(`* Executed ${cmd} commands`.green);
}
