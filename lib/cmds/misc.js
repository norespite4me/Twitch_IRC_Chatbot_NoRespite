...
export function squirt(channel, user, cmd, args, client) {
    ...
    if (!args[0]) {
        if (user.username == 'intercreates') {
            client.say(channel, `/me ${user.username}'s squirt distance is 50 cm!  interyPOP`);
        } else {
            client.say(channel, `/me ${user.username}'s squirt distance is ${randy} cm! ${randemote()}`);
        }

    } else {
        if (args[0] == 'intercreates' || args[0] == '@intercreates') {
            client.say(channel, `/me ${args[0]}'s squirt distance is 50 cm! interyPOP`);
        } else {
            client.say(channel, `/me ${args[0]}'s squirt distance is approximately ${randy} cm! ${randemote()}`);
        }
    }
    console.log(`* Executed ${cmd} command`.green);
}
